...
static NLboolean sock_ConnectUDPAsynch(NLsocket socket, const NLaddress *address)
{
    ...
    (void)nlThreadCreate(sock_ConnectUDPAsynchInt, (void *)addr, NL_FALSE);
    return NL_TRUE;
}
...
NLchar *sock_AddrToString(const NLaddress *address, NLchar *string)
{
    ...
    if(port == 0)
    {
        _stprintf(string, (const NLchar *)TEXT("%lu.%lu.%lu.%lu"), (addr >> 24) & 0xff, (addr >> 16)
            & 0xff, (addr >> 8) & 0xff, addr & 0xff);
    }
    else
    {
        _stprintf(string, (const NLchar *)TEXT("%lu.%lu.%lu.%lu:%u"), (addr >> 24) & 0xff, (addr >> 16)
...
NLboolean sock_StringToAddr(const NLchar *string, NLaddress *address)
{
    ...
    ret = _stscanf((const NLchar *)string, (const NLchar *)TEXT("%lu.%lu.%lu.%lu:%hu"), &a1, &a2, &a3, &a4, &port);
...
{
    /* should we check against all the local adderesses? */
...
NLchar *sock_GetNameFromAddr(const NLaddress *address, NLchar *name)
{
    ...
    if(hostentry != NULL)
    {
        ...
        if(port != 0)
        {
            _sntprintf(name, (size_t)(NL_MAX_STRING_LENGTH), (const NLchar *)TEXT("%s:%hu"), (const NLchar *)temp, port);
        }
        else
        {
            _tcsncpy(name, (const NLchar *)temp, (size_t)(NL_MAX_STRING_LENGTH));
        }
        name[NL_MAX_STRING_LENGTH - 1] = (NLbyte)'\0';
    }
    else
    {
        if(((struct sockaddr_in *)address)->sin_addr.s_addr == (unsigned long)INADDR_NONE)
        {
            _tcscpy(name, (const NLchar *)TEXT("Bad address"));
        }
        else
        {

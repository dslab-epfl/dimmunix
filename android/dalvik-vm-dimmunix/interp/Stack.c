...
int saveFrame(char* frames, int start, const char* className, const char* methodName, int line) {
	int pos = start;
	int i;

	for (i = 0; className[i]; i++) {
		frames[pos++] = className[i];
	}

	frames[pos++] = '.';

	for (i = 0; methodName[i]; i++) {
		frames[pos++] = methodName[i];
	}

	frames[pos++] = '(';
	int l = itoa(line, frames+ pos);
	pos += l;
	frames[pos++] = ')';
	frames[pos++] = '\0';
...
int dvmGetCallStack(Thread *thread, const int MaxFrames)
{
	char* frames = thread->stackBuffer;
	char* className = thread->classNameBuffer;
	...
	while (framePtr != NULL && frameIndex < MaxFrames) {
		saveArea = SAVEAREA_FROM_FP(framePtr);
		method = saveArea->method;
		...
		if (method == NULL) {
			...
			dvmDescriptorToDotEfficient(method->clazz->descriptor, className);

			if (frameIndex > 0) {
				strcat(frames, ",");
			}
			if (dvmIsNativeMethod(method)){
				sprintf(frames+strlen(frames), "%s.%s(Native Method)", className, method->name);
			}
			else {
				int line = relPc < 0 ? -1 : dvmLineNumFromPC(method, relPc);
				pos = saveFrame(frames, pos, className, method->name, line);
//				sprintf(frames+strlen(frames), "%s.%s(%d)", className, method->name, line);
			}
	...
	//a local variable on the native stack.
		...
		if (saveArea->prevFrame != NULL && saveArea->prevFrame <= framePtr) {
			LOGW("Warning: loop in stack trace at frame (%p -> %p)\n", framePtr, saveArea->prevFrame);
			break;
		}
		framePtr = saveArea->prevFrame;
	}
	//LOGD("DIMMUNIX : returning callStack");
	return frameIndex;
}

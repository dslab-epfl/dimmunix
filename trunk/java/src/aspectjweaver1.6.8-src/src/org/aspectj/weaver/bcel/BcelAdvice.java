...
class BcelAdvice extends Advice {
	...
	private StackTraceElement getPosition(LazyMethodGen synchronizedMethod) {
		String className = synchronizedMethod.getClassName();
		String methodName = synchronizedMethod.getName();
		String fileName = synchronizedMethod.getEnclosingClass().getFileName();
		int lineNumber = 0;
		
		InstructionList body = synchronizedMethod.getBodyForPrint();
		InstructionHandle walker = body.getStart();
		while (walker != null) {
			for (InstructionTargeter targeter: walker.getTargetersCopy()) {
				if (targeter instanceof LineNumberTag) {
					lineNumber = ((LineNumberTag)targeter).getLineNumber();
					break;
				}
			}
			if (lineNumber == 0)
	...
	public boolean match(Shadow shadow, World world) {
		...
		//for dimmunix 
		if (ret) {
			BcelShadow bshadow = (BcelShadow)shadow;
			if (shadow.getKind() == Shadow.SynchronizationLock || shadow.getKind() == Shadow.SynchronizationUnlock || bshadow.getEnclosingMethod().isSynchronized() && shadow.getKind() == Shadow.MethodExecution) {
				StackTraceElement methPos = this.getPosition(bshadow.getEnclosingMethod());
				StackTraceElement pos = bshadow.getPosition();

				if (SyncPositions.instrumentAll) {
					return true;
				}
				if (shadow.getKind() == Shadow.MethodExecution) {
					//begin exec sync method
					return SyncPositions.classes.contains(pos.getClassName()) && SyncPositions.methods.contains(pos.getMethodName());
				}
				if (shadow.getKind() == Shadow.SynchronizationLock) {
					return SyncPositions.syncPositions.contains(pos);
				}
				if (shadow.getKind() == Shadow.SynchronizationUnlock) {
					if (bshadow.getEnclosingMethod().isSynchronized() && SyncPositions.syncPositions.contains(methPos) && bshadow.getEnclosingMethod().monExits.contains(bshadow.instruction)) {
			...
			if (shadow.getKind() == Shadow.MethodCall) {
				Member sig = bshadow.getSignature();
				String cls = sig.getDeclaringType().getName();
				String meth = sig.getName();
				
				if (cls.equals("java.util.concurrent.locks.ReentrantLock") && (meth.equals("lock") || meth.equals("unlock")) ||
						cls.equals("java.util.concurrent.locks.ReentrantReadWriteLock$ReadLock") && (meth.equals("lock") || meth.equals("unlock")) || 
						cls.equals("java.util.concurrent.locks.ReentrantReadWriteLock$WriteLock") && (meth.equals("lock") || meth.equals("unlock")) ||
						cls.equals("java.util.concurrent.Semaphore") && (meth.equals("acquire") || meth.equals("release")) ||
						cls.equals("java.nio.channels.FileChannel") && meth.equals("lock") ||
						cls.equals("java.nio.channels.FileLock") && meth.equals("release")) {
					StackTraceElement pos = new StackTraceElement(bshadow.getEnclosingClass().getName(), bshadow.getEnclosingMethod().getName(), bshadow.getEnclosingClass().getFileName(), bshadow.getSourceLocation().getLine());
					ExecutionPositions.instance.addPosition(pos);
				}					
			}
		}
		return ret;

...
class BcelClassWeaver implements IClassWeaver {
	...
	public static void transformSynchronizedMethod(LazyMethodGen synchronizedMethod) {
		if (!SyncPositions.instrumentAll && (!SyncPositions.classes.contains(synchronizedMethod.getClassName()) || !SyncPositions.methods.contains(synchronizedMethod.getName())))
			return;
		...
		if (synchronizedMethod.isStatic()) {
			...
			if (synchronizedMethod.getEnclosingClass().isAtLeastJava5()) {
				...
				synchronizedMethod.monExits.add(finallyBlock.append(InstructionConstants.MONITOREXIT));//dimmunix
				...
				if (!rets.isEmpty()) {
					...
					for (Iterator iter = rets.iterator(); iter.hasNext();) {
						...
						synchronizedMethod.monExits.add(monitorExitBlock.append(InstructionConstants.MONITOREXIT));//dimmunix

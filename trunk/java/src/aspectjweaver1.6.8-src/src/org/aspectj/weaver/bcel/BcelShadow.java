...
public class BcelShadow extends Shadow {
	...
	public InstructionHandle instruction;//dimmunix
	...
	public static BcelShadow makeMonitorExit(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle monitorInstruction,
		...
		s.instruction = monitorInstruction;
		ShadowRange r = new ShadowRange(body);
		r.associateWithShadow(s);
		r.associateWithTargets(Range.genStart(body, monitorInstruction), Range.genEnd(body, monitorInstruction));
		retargetAllBranches(monitorInstruction, r.getStart());
	...
	public ISourceLocation getSourceLocation() {
		...
		if (sourceLine == 0 || sourceLine == -1) {
			...
			if (getKind() == Shadow.SynchronizationLock) {
				int line = this.getEnclosingMethod().getMemberView().getLineNumberOfFirstInstruction();
				return getEnclosingClass().getType().getSourceContext().makeSourceLocation(line, 0);
			}	
	...
	public StackTraceElement getPosition() {
		return new StackTraceElement(this.getEnclosingClass().getClassName(), this.getEnclosingMethod().getName(), this.getEnclosingClass().getFileName(), this.getSourceLocation().getLine());
	}

...
static void lockMonitor(Thread* self, Monitor* mon)
{
    ...
    struct Node* lnode = &mon->node;
    struct Node* tnode = &self->node;
    before(self, tnode, lnode);//dimmunix
    ...
    after(self, tnode, lnode);//dimmunix
}
...
static bool unlockMonitor(Thread* self, Monitor* mon)
{
    ...
    if (mon->owner == self) {
        ...
        if (mon->lockCount == 0) {
            ...
            struct Node* lnode = &mon->node;
            struct Node* tnode = &self->node;
            beforeUnlock(tnode, lnode);//dimmunix
...
extern pthread_mutex_t avoidanceLock;
    ...
    bool interruptShouldThrow)
{
    ...
    if (mon == NULL) {//create the Monitor
    	pthread_mutex_lock(&avoidanceLock);
...
void dvmObjectNotify(Thread* self, Object *obj)
{
    ...
    if (LW_SHAPE(thin) == LW_SHAPE_THIN) {
        ...
        /* It's a fat lock.
         */
        notifyMonitor(self, LW_MONITOR(obj->lock));
    }
}
...
void dvmObjectNotifyAll(Thread* self, Object *obj)
{
    ...
    if (LW_SHAPE(thin) == LW_SHAPE_THIN) {
        ...
        /* It's a fat lock.
         */
        notifyAllMonitor(self, LW_MONITOR(obj->lock));
    }
}
